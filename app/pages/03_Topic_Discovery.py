...
def top_n_topics(model, df, keywords, topic_granularity):
    ...
    if keywords != "":
        if keywords in model.vocab:
            ...
            subset_topics = model.search_topics(keywords=tokenized_kw, num_topics = topic_granularity, reduced=red_status)[3]
    ...
    for topic_num in subset_ranked_topics:
        ...
        dict_gridtable[topic_num] = AgGrid(subset_of_df_in_topic, height=300, gridOptions=gridoptions, fit_columns_on_grid_load=True,
...
if "df_after_form_completion" in st.session_state:
    ...
    intermediate_topics_and_unlabelled_df = pd.concat([st.session_state["df_after_form_completion"], unlabelled_data], ignore_index = True)
    ...
    if manual_labelling_button:
        ...
        selected_topic_labels = intermediate_labelled_topics_df["Index"].unique().tolist()
        df_filtered = st.session_state['df_filtered']
