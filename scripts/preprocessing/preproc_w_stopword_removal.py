...
def full_preprocessing(data, col):
    ...
    filtered['tokenized'] = full_lemmatization_df(filtered, 'lowercased')
    filtered['clean_content'] = remove_stopwords_df(filtered, 'tokenized')
    return filtered.drop(['removed_punc', 'lowercased', 'tokenized'], axis = 1)