...
format.align_text(
    """
    In this page, you can verify if the predicted indexes and subindexes are correct. 
...
def run():
    if utils.check_session_state_key("csv_file_filtered"):
        if utils.check_session_state_key("csv_file_with_predicted_labels"):
            ...
            if utils.check_session_state_key("current_index_index"):
            ...
            display_aggrid_by_index(table_collection, current_index_index)

            format.horizontal_line()
        else:
            utils.customDisppearingMsg(
                "Please run the Index Model Simulator first!",
                wait=-1,
                type_="warning",
                icon="⚠️",
            )
